...
//        claims.put("password", userDetails.getPassword());
        claims.put("roles", userDetails.getAuthorities().stream().map(GrantedAuthority::toString).collect(Collectors.joining(",")));
...
//        final String role = this.getRoleFromToken(token);
        final Date created = this.getCreatedDateFromToken(token);
